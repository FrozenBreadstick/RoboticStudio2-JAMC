...
Click the "Select A MIDI File" button.

    This opens a file dialog.

    Selecting a .mid file triggers the MidiProcessor to convert it to a .mipi file.

    Note: Processed files are automatically saved to ~/mipi_files/.

## Step 2: Loading an Existing .mipi File

Use the Dropdown Menu (QComboBox).

    This scans the ~/mipi_files/ directory.

    Selecting a file automatically populates the Channel Selector with the instruments found in that file.

## Step 3: Selecting a Channel

Click a Radio Button in the Channel Selector.

    This immediately sends a /MIPI/load service request to the robot.

    It passes the .mipi filepath and the specific MIDI channel index.
